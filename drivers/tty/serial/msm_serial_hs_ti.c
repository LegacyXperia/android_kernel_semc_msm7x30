				...
				unsigned long flags)
{
	...
	if (curr_uartclk != uport->uartclk) {
		if (clk_set_rate(msm_uport->clk, uport->uartclk)) {
			pr_err("[BT]%s(): Error setting clock rate on UART\n",
								__func__);
			WARN_ON(1);
			spin_lock_irqsave(&uport->lock, flags);
			return flags;
...
static void msm_hs_shutdown(struct uart_port *uport)
{
	...
	printk(KERN_INFO "[BT] Shutdown start\n");
	...
	printk(KERN_INFO "[BT] Shutdown end\n");
}
...
static int msm_hs_runtime_resume(struct device *dev)
{
	struct platform_device *pdev = container_of(dev, struct
						    platform_device, dev);
	struct msm_hs_port *msm_uport = &q_uart_port[pdev->id];
	ti_msm_hs_request_clock_on(&msm_uport->uport);
	return 0;
}
...
static int msm_hs_runtime_suspend(struct device *dev)
{
	struct platform_device *pdev = container_of(dev, struct
						    platform_device, dev);
	struct msm_hs_port *msm_uport = &q_uart_port[pdev->id];
	ti_msm_hs_request_clock_off(&msm_uport->uport);
	return 0;
}
